...
from pytest_mongo_docker.utils import resolve_docker_host


def test_returns_env_when_set(monkeypatch: Any) -> None:
    monkeypatch.setenv("DOCKER_HOST", "tcp://example:2375")
    with mock.patch("pytest_mongo_docker.utils.subprocess.run") as run:
...
def test_returns_none_when_cli_missing(monkeypatch: Any) -> None:
    ...
    with mock.patch("pytest_mongo_docker.utils.shutil.which", return_value=None):
...
def test_reads_active_context(monkeypatch: Any) -> None:
    ...
    with (
        mock.patch("pytest_mongo_docker.utils.shutil.which", return_value="/usr/bin/docker"),
        mock.patch("pytest_mongo_docker.utils.subprocess.run", return_value=completed) as run,
    ):
...
def test_returns_none_on_empty_output(monkeypatch: Any) -> None:
    ...
    with (
        mock.patch("pytest_mongo_docker.utils.shutil.which", return_value="/usr/bin/docker"),
        mock.patch("pytest_mongo_docker.utils.subprocess.run", return_value=completed),
    ):
        assert resolve_docker_host() is None
...
def test_returns_none_on_cli_error(monkeypatch: Any) -> None:
    ...
    with (
        mock.patch("pytest_mongo_docker.utils.shutil.which", return_value="/usr/bin/docker"),
        mock.patch("pytest_mongo_docker.utils.subprocess.run", side_effect=err),
    ):
        assert resolve_docker_host() is None
